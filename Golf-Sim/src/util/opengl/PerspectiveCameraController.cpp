...
PerspectiveCameraController::PerspectiveCameraController(
    glm::vec3& cameraPos, float yaw, float pitch, float fov, float aspectRatio,
    float moveSpeed, float zoomSpeed, float panSensitivity)
    : camera(cameraPos, yaw, pitch, fov, aspectRatio),
      moveSpeed(moveSpeed),
      zoomSpeed(zoomSpeed),
...
void PerspectiveCameraController::update(GLCore::Timestep ts) {
  ...
  if (GLCore::Input::IsKeyPressed(HZ_KEY_W)) {
    cameraPos += glm::vec3(cos(glm::radians(yaw)), 0, sin(glm::radians(yaw))) *
                 deltaTime * moveSpeed;
  ...
  if (GLCore::Input::IsKeyPressed(HZ_KEY_A)) {
    cameraPos -=
        glm::normalize(glm::cross(cameraFront, glm::vec3(0.0, 1.0, 0.0))) *
        deltaTime * moveSpeed;
...
bool PerspectiveCameraController::OnMouseScrolled(
    GLCore::MouseScrolledEvent& e) {
  camera.setFOV(camera.getFOV() + e.GetYOffset() * zoomSpeed);
  return false;
}

bool PerspectiveCameraController::OnWindowResized(
    GLCore::WindowResizeEvent& e) {
