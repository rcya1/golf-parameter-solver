...
// from https://cp-algorithms.com/geometry/circle-line-intersection.html#solution
// assumes circle is centered at origin and the line is giving in the form ax + by + c = 0
std::vector<glm::vec2> circleLineIntersectionOrigin(float r, float a, float b, float c) {
	float EPS = 0.01;
	...
	for (int row = rb; row < rt; row++) {
		for (int col = rl; col < rr; col++) {
			...
			std::vector<glm::vec3> topIntersections = addHeights(
				circleLineIntersection(goalCenter, radius, get2D(topLeft), get2D(topRight)),
				terrain);
			std::vector<glm::vec3> rightIntersections = addHeights(
				circleLineIntersection(goalCenter, radius, get2D(topRight), get2D(botRight)),
				terrain);
			std::vector<glm::vec3> botIntersections = addHeights(
				circleLineIntersection(goalCenter, radius, get2D(botLeft), get2D(botRight)),
				terrain);
			std::vector<glm::vec3> leftIntersections = addHeights(
				circleLineIntersection(goalCenter, radius, get2D(botLeft), get2D(topLeft)),
				terrain);
			...
			innerPoints.insert(innerPoints.end(), borderPoints.begin(), borderPoints.end());
			...
			// patch up holes by adding triangles between points on cell borders and their nearest neighbors
			for (glm::vec3& x : borderPoints) {
				auto sortDist = [x](const GoalModelPoint& a, const GoalModelPoint& b) {
					return glm::length(x - glm::vec3(a.pos.x, a.height, a.pos.y)) < glm::length(x - glm::vec3(b.pos.x, b.height, b.pos.y));
				};

				std::sort(pointsCopy.begin(), pointsCopy.end(), sortDist);

				glm::vec3 a = glm::vec3(pointsCopy[0].pos.x, pointsCopy[0].height, pointsCopy[0].pos.y);
				glm::vec3 b = glm::vec3(pointsCopy[1].pos.x, pointsCopy[1].height, pointsCopy[1].pos.y);

				//printVec3(-glm::vec3(5, 5, 5) + x);
				//printVec3(-glm::vec3(5, 5, 5) + a);
				//printVec3(-glm::vec3(5, 5, 5) + b);
				//printf("\n");
				addTriangle(x, a, b);
			}
		}
